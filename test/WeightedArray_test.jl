@testset "WeightedArray.jl" begin
    ...
    @test WeightedData.value(A) == [1.0, 2.0]
    @test WeightedData.precision(A) == [0.5, 0.5]
    ...
    @test value(weighted_array) == A
    @test precision(weighted_array) == B
    ...
    @test @inferred(value(weighted_array)) == A
    @test @inferred(precision(weighted_array)) == B
    ...
    @test value(flagged_array) == [1.0, 0.0, 3.0]
    @test precision(flagged_array) == [0.1, 0.0, 0.3]

    flagbaddata!(weighted_array, badpix)
    @test value(weighted_array) == [1.0, 0.0, 3.0]
    @test precision(weighted_array) == [0.1, 0.0, 0.3]
...
@testset "WeightedArray arithmetic operations" begin
    ...
    @test value(C) ≈ [4.0, 6.0]
    @test precision(C) ≈ [0.25, 0.16] # inv(inv(0.5)+inv(0.5)) = 0.25, inv(inv(0.2)+inv(0.8)) = 0.16
    @test C == A .+ B
    ...
    @test D == A .- B
    @test value(D) ≈ [-2.0, -2.0]
    @test precision(D) ≈ [0.25, 0.16]

    # Test WeightedArray + Number
    E = A + 2.0
    @test E isa WeightedArray
    @test E == A .+ 2.0
    @test value(E) ≈ [3.0, 4.0]
    @test precision(E) == [0.5, 0.2]

    # Test Number + WeightedArray
    F = 2.0 + A
    @test F isa WeightedArray
    @test F == 2.0 .+ A
    @test value(F) ≈ [3.0, 4.0]
    @test precision(F) == [0.5, 0.2]

    # Test WeightedArray - Number
    G = A - 1.0
    @test G isa WeightedArray
    @test G == A .- 1.0
    @test value(G) ≈ [0.0, 1.0]
    @test precision(G) == [0.5, 0.2]

    # Test Number - WeightedArray
    H = 5.0 - A
    @test H isa WeightedArray
    @test H == 5.0 .- A
    @test value(H) ≈ [4.0, 3.0]
    @test precision(H) == [0.5, 0.2]

    # Test WeightedArray / Number
    I = A / 2.0
    @test I isa WeightedArray
    @test I == A ./ 2.0
    @test value(I) ≈ [0.5, 1.0]
    @test precision(I) ≈ [2.0, 0.8] # 2^2 * precision

    # Test Number / WeightedArray
    J = 2.0 / A
    @test J isa WeightedArray
    @test J == 2.0 ./ A
    @test value(J) ≈ [2.0, 1.0]
    @test precision(J) ≈ [1.0 / (0.5 * 4.0), 1.0 / (0.2 * 4.0)] # inv(precision) / 2^2

    # Test Number * WeightedArray
    K = 3.0 * A
    @test K isa WeightedArray
    @test K == 3.0 .* A
    @test value(K) ≈ [3.0, 6.0]
    @test precision(K) ≈ [0.5 / 9.0, 0.2 / 9.0] # precision / 3^2

    # Test WeightedArray * Number
    L = A * 3.0
    @test L isa WeightedArray
    @test L == A .* 3.0
    @test value(L) ≈ [3.0, 6.0]
    @test precision(L) ≈ [0.5 / 9.0, 0.2 / 9.0]
...
@testset "WeightedArray arithmetic with   arrays" begin
    ...
    arr = [2.0, 3.0]
    res = A + arr
    @test value(res) == [3.0, 5.0]
    @test precision(res) == [0.5, 0.2]


    # Array + WeightedArray
    @test value(arr + A) == [3.0, 5.0]
    @test precision(arr + A) == [0.5, 0.2]
    @test (arr + A) == arr .+ A

    # WeightedArray - Array
    @test value(A - arr) == [-1.0, -1.0]
    @test precision(A - arr) == [0.5, 0.2]
    @test (A - arr) == A .- arr

    # Array - WeightedArray
    @test value(arr - A) == [1.0, 1.0]
    @test precision(arr - A) == [0.5, 0.2]
    @test (arr - A) == arr .- A
    ...
    arr2 = [2.0, 4.0]
    res2 = A ./ arr2
    @test value(res2) == [0.5, 0.5]
    @test precision(res2) ≈ [2.0, 3.2]

    # Array / WeightedArray
    arr3 = [2.0, 4.0]
    res3 = arr3 ./ A
    @test value(res3) == [2.0, 2.0]
    @test precision(res3) ≈ [0.5, 0.3125]

    # Array * WeightedArray
    arr4 = [2.0, 4.0]
    res4 = arr4 .* A
    @test value(res4) == [2.0, 8.0]
    @test precision(res4) ≈ [0.125, 0.0125]
